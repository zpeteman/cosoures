...
\begin{center}
    ...
    % Coordinate axes
    \draw[->, thick] (0,0) -- (2.25,0) node[right] {$\vec{y}_0$};
    \draw[->, thick] (0,0) -- (0,2.5) node[above] {$\vec{z}_0$};
    \draw[->, thick] (0,0) -- (-1.2,-1.2) node[below] {$\vec{x}_0$};
    
    % Vector T (upper left direction)
    \draw[->, thick] (0,0) -- (-1.8,2.2) node[pos=0.9, below left] {$\vec{T}$};
    
    % Vector u (lower right direction)
    \draw[->, thick] (0,0) -- (0.4,-1.3) node[right] {$\vec{u}$};
    
    % Angle theta (between z-axis and T vector)
    \draw (0,0.5) arc (-110:-0.7:-0.3);
    \node at (-0.25,0.7) {$\theta$};
    
    % Angle psi (between x_0 and u vector)
    \draw (-0.5,-0.5) arc (-135:-73:0.7);
    \node at (-0.3,-0.8) {$\psi$};
    
    % Circle (disk) with center G
    \coordinate (G) at (-1,1.2);
    \draw[thick] (G) circle (0.4);
    \fill[red] (G) circle (0.05);
    \node[left] at (G) {$G$};
    \node at (-1,0.65) {$(S)$};
    
    % Angle phi at the contact point
    \draw[->] (-1,1.65) arc (-96:-3:-0.4);
    \node at (-1.2,1.75) {$\phi$};
